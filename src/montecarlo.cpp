 ...
 //   debug << "Entering emit_principal_variation() ..." << endl;
    ...
    if (n > 0)
    {
        ...
        while (pos.legal(move))
        {
			...
			while ((legalMove = mp.next_move()) != MOVE_NONE)
				if (pos.legal(legalMove))
				{
					if(legalMove == move)
					{
						trueMove = true;
...
double MonteCarlo::UCB(mctsNode node, Edge& edge, bool priorMode) {
    ...
    if (edge.visits > Options["MinVisits"])
